...
const _SB={url:"https://mhrtzppoiinpnbnximuf.supabase.co",key:"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1ocnR6cHBvaWlucG5ibnhpbXVmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ4OTE3MDEsImV4cCI6MjA5MDQ2NzcwMX0.933qWXp0vslGHmt06eKgPuihMOVh4NzGUiHXY4iDNSQ",headers:{"apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1ocnR6cHBvaWlucG5ibnhpbXVmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ4OTE3MDEsImV4cCI6MjA5MDQ2NzcwMX0.933qWXp0vslGHmt06eKgPuihMOVh4NzGUiHXY4iDNSQ","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1ocnR6cHBvaWlucG5ibnhpbXVmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ4OTE3MDEsImV4cCI6MjA5MDQ2NzcwMX0.933qWXp0vslGHmt06eKgPuihMOVh4NzGUiHXY4iDNSQ","Content-Type":"application/json"},async get(t,q){try{const r=await fetch(this.url+"/rest/v1/"+t+"?"+(q||"select=*"),{headers:this.headers});if(!r.ok)return null;return r.json();}catch(e){return null;}},async upsert(t,rows,c){if(!rows||!rows.length)return;try{const r=await fetch(this.url+"/rest/v1/"+t,{method:"POST",headers:{...this.headers,"Prefer":"resolution=merge-duplicates,return=minimal","Content-Type":"application/json"},body:JSON.stringify(rows)});if(!r.ok){console.warn("Supabase sync skipped:",t,r.status);}}catch(e){console.warn("Supabase sync error:",t);}}};
...
async function sbSyncStudents(s){await _SB.upsert("students",s.map(x=>({id:parseInt(String(x.id)),sid:x.sid,name:x.name,class_id:x.classId,gender:x.gender||null,phone:x.phone||null,status:x.status||"active",academic_year:x.academicYear||null})),"id");}
...
function Teachers({ userRole, classes = [] }) {
  ...
  const printTeacherReport = (teacher) => {
    ...
    win.document.write(`<!DOCTYPE html><html><head><title>Teacher Report</title><style>body{font-family:serif;padding:60px;max-width:800px;margin:0 auto}table{width:100%;border-collapse:collapse;margin:16px 0}td{padding:9px 14px;border:1px solid #d1d5db;font-size:13px}td:first-child{background:#f9fafb;font-weight:bold;width:38%}.footer{margin-top:40px;text-align:center;font-size:11px;color:#9ca3af;border-top:1px solid #e5e7eb;padding-top:12px}@media print{button{display:none!important}}</style></head><body>
    <div style="border-bottom:3px solid #0d9488;padding-bottom:16px;margin-bottom:30px"><div style="font-size:22px;font-weight:bold;color:#0d9488">Al-Huffath Academy</div><div style="font-size:12px;color:#64748b">Ilm | Iman | Hifz</div></div>
    <p>${today}</p><p><b>OBJECT: Confirmation of Employment</b></p>
    <table>
      ...
      <tr><td>Username</td><td>${teacher.username||"-"}</td></tr>
    </table>
    <div class="footer">Generated by EduManage | ${today}</div>
    <button onclick="window.print()" style="padding:10px 28px;background:#0d9488;color:#fff;border:none;border-radius:6px;cursor:pointer;margin-top:16px">Print</button>
    </body></html>`);
    win.document.close();
  };
    ...
    <div>
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.radius, overflow: "hidden", boxShadow: T.cardShadow }}>
        <div style={{ padding: "14px 18px", borderBottom: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: T.textMain }}>All Teachers</div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", direction: "ltr" }}>
            <thead>
              <tr style={{ background: "#f8fafc" }}>
                {["Teacher", "Subject", "Email", "Phone", "Classes", "Status", "Actions"].map(h => (
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={t.id} onMouseEnter={e => e.currentTarget.style.background = T.rowHover} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                  ...
                  <td style={{ padding: "12px 16px", borderBottom: "1px solid #f8fafc" }}>
                    <div style={{ display: "flex", gap: 6 }}>
                      {userRole === "admin" && <>
                        <button onClick={() => { setEditing(t); setShowForm(true); }} style={{ padding: "5px 12px", borderRadius: 6, border: `1px solid ${T.border}`, background: "#fff", fontSize: 12, cursor: "pointer", color: "#334155" }}>Edit</button>
                      ...
                      <button onClick={() => printTeacherReport(t)} style={{ padding: "5px 12px", borderRadius: 6, border: "none", background: "#d1fae5", fontSize: 12, cursor: "pointer", color: "#065f46" }}>Report</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {showForm && (
        <Modal title={editing ? "Edit Teacher" : "Add New Teacher"} onClose={() => { setShowForm(false); setEditing(null); }}>
          <form onSubmit={handleSave}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 16px" }}>
              <div style={{ gridColumn: "1/-1" }}>
                <Field label="Full Name"><input name="tname" defaultValue={editing?.name} required style={inputStyle} placeholder="Ms. Jane Doe" /></Field>
              </div>
              <Field label="Subject"><input name="subject" defaultValue={editing?.subject} style={inputStyle} placeholder="Mathematics" /></Field>
              <Field label="Phone"><input name="phone" defaultValue={editing?.phone} style={inputStyle} placeholder="555-0000" /></Field>
              <Field label="Email"><input name="email" type="email" defaultValue={editing?.email} style={inputStyle} placeholder="teacher@school.edu" /></Field>
              <Field label="Status">
                <select name="status" defaultValue={editing?.status || "Active"} style={selectStyle}>
            ...
            {classes.length > 0 && (
              <Field label="Assigned Classes">
                <div style={{ border: `1px solid ${T.border}`, borderRadius: 8, padding: 10, maxHeight: 120, overflowY: "auto", display: "flex", flexDirection: "column", gap: 4 }}>
                  {classes.map(cls => (
                    <label key={cls.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, cursor: "pointer" }}>
                      <input type="checkbox" name="classIds" value={cls.id}
                        defaultChecked={(editing?.classIds||editing?.class_ids||[]).map(x=>parseInt(x)).includes(cls.id)} />
                      {cls.name}
                    </label>
                  ))}
                </div>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 8 }}>
              <button type="button" onClick={() => { setShowForm(false); setEditing(null); }} style={{ padding: "9px 18px", borderRadius: 8, border: `1px solid ${T.border}`, background: "#fff", fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
              <button type="submit" style={{ padding: "9px 24px", borderRadius: 8, border: "none", background: T.primary, color: "#fff", fontSize: 13, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>
      ...
      {deleteId && (
        <Modal title="Delete Teacher?" onClose={() => setDeleteId(null)}>
          <p style={{ fontSize: 14, color: T.textSub, marginBottom: 24 }}>Are you sure you want to delete <strong>{teachers.find(t => t.id === deleteId)?.name}</strong>?</p>
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
            ...
            <button onClick={() => handleDelete(deleteId)} style={{ padding: "9px 22px", borderRadius: 8, border: "none", background: T.danger, color: "#fff", fontSize: 13, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>Delete</button>
          </div>
        </Modal>
      )}
    </div>
