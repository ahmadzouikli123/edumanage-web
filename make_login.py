...
os.makedirs("src/app/school/login", exist_ok=True)
...
export default function Login() {
  ...
  const go = () => {
    ...
    setTimeout(() => {
      ...
      if (role === "admin") {
        ...
        const s  = ss.find(x => x.sid.toLowerCase()===user.toLowerCase().trim() && x.phone.replace(/\\\\D/g,"")===pass.replace(/\\\\D/g,""))
  ...
  const inp = { width:"100%", padding:"11px 14px", borderRadius:10, border:"1px solid rgba(255,255,255,.15)", background:"rgba(255,255,255,.07)", color:"#fff", fontSize:14, fontFamily:"inherit", outline:"none", boxSizing:"border-box" }
...
with open("src/app/school/login/page.tsx", "w", encoding="utf-8", newline="") as f:
    f.write(content)
